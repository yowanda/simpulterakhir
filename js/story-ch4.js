...
'ch4_hunt_key': {
  text: (s) => {
    let text = `<p class="sound">— Kau memimpin kelompok kecil untuk mencari kunci master. —</p>`;

    if (s.flags.hasMansionMap) {
      text += `<p><span class="speaker farah">Farah</span> Diagram mansion yang kau temukan menunjukkan satu ruangan yang tidak ada di blueprint resmi — ruangan antara dinding di sayap timur lantai dua. Hidden room.</p>`;
      text += `<p><span class="speaker farah">Farah</span> "Di sini," kau menunjuk peta. "Ruangan ini tidak ada di denah. Kalau aku Sang Penenun, di situlah aku simpan kunci."</p>`;
    } else {
      text += `<p><span class="speaker farah">Farah</span> Tanpa peta, kau harus mencari secara manual. Setiap ruangan, setiap laci, setiap panel yang bisa dipindahkan.</p>`;
    }
...
'ch4_hidden_room': {
  text: (s) => {
    let text = `<p class="sound">— Sayap timur lantai dua. Dinding yang menurut peta menyembunyikan ruangan. —</p>`;
    text += `<p class="sound">— Kau mengetuk. Hollow. Ada ruang di baliknya. —</p>`;
    text += `<p><span class="speaker juno">Juno</span> menemukan mekanismenya — panel geser yang tersembunyi di balik lukisan.</p>`;
    text += `<p><span class="speaker juno">Juno</span> Panel bergeser. Lorong sempit membawa kalian ke ruangan kecil, tidak lebih besar dari walk-in closet.</p>`;
    text += `<p class="sound">— Di dalamnya: —</p>`;
...
'ch4_hide_listen': {
  text: (s) => {
    ...
    if (s.difficulty === 3) {
      ...
      text += `<p><span class="speaker dimas">Dimas</span> "Target utama prioritas. Yang lain secondary." Suara tunggal — terdistorsi, tapi ada nada yang familiar. Kau belum bisa mengidentifikasi.</p>`;
    }

    text += `<p class="sound">— Kau merekam percakapan ini di ponselmu. Bukti. —</p>`;
...
'ch4_open_box': {
  text: `<p class="sound">— 1971. Klik. Kotak terbuka. —</p>
<p><span class="speaker dimas">Dimas</span> Di dalam: kunci master mansion — besar, kuningan tua, dengan symbol simpul terukir. Dan sesuatu lagi — USB drive dan sepucuk surat.</p>
<p><span class="speaker dimas">Dimas</span> Surat itu: <em>"Bagi yang menemukan ini — selamat. Kau sudah cukup dekat dengan kebenaran untuk menjadi ancaman. Kunci ini membuka pintu keluar utama. Tapi hanya bisa digunakan sekali — setelah diputar, mekanisme rusak permanen. Gunakan dengan bijak. Atau simpan sebagai asuransi."</em></p>
<p><span class="speaker dimas">Dimas</span> Satu kunci. Satu kesempatan keluar. Untuk semua orang? Atau hanya untuk yang paling cepat?</p>`,
  choices: [
    {
...
'ch4_mass_escape': {
  text: `<p class="sound">— Semua bergerak ke pintu. Tapi sebelum orang terakhir keluar— —</p>
<p><span class="speaker juno">Juno</span> BLAM. Pintu menutup kembali dari luar. Mekanisme — yang seharusnya rusak — ternyata punya backup. Hanya enam orang yang berhasil keluar.</p>
<p class="sound">— Yang terjebak di dalam: kau, dan beberapa orang lain. —</p>
<p><span class="speaker juno">Juno</span> Speaker: <em>"Evakuasi parsial. Menarik. Tapi permainan belum selesai untuk yang tersisa."</em></p>
<p class="sound">— Yang berhasil keluar berlari ke hutan. Yang tersisa menatap pintu tertutup. —</p>`,
  shake: true,
  choices: [
    {
...
'ch4_hunt_penenun': {
  text: (s) => {
    let text = `<p><span class="speaker juno">Juno</span> "Kunci master bisa jebakan. Yang kita butuhkan bukan jalan keluar — tapi menghentikan orang yang mengontrol keluar-masuk."</p>`;
    text += `<p class="sound">— Kau menyusun profil Sang Penenun dari semua clue: —</p>`;
    text += `<p class="sound">— • Mengenal sejarah mansion selama 50+ tahun —</p>`;
    text += `<p class="sound">— • Punya akses ke file polisi, data medis, dan informasi pribadi semua tamu —</p>`;
    text += `<p class="sound">— • Menggunakan operator — "Penulis" dan "Pemotong" —</p>`;
    text += `<p class="sound">— • Bisa berada di mansion tanpa terdeteksi — atau SUDAH di antara tamu —</p>`;

    if (s.flags.hasJournal) {
      text += `<p><span class="speaker niko">Niko</span> • Kakek Niko menyebut dia sebagai partner yang mem-blackmail selama 48 tahun</p>`;
    }
    ...
    if (s.difficulty === 1) {
      text += `<p><span class="speaker sera">Sera</span> "Satu pembunuh," katamu. "Satu orang yang menarik semua benang. Siapa?"</p>`;
    } else if (s.difficulty === 2) {
      text += `<p><span class="speaker sera">Sera</span> "Dua operator setidaknya. Plus dalang di atas mereka — atau di antara mereka."</p>`;
    } else {
      text += `<p><span class="speaker sera">Sera</span> "Konspirasi tiga orang. Masing-masing punya peran. Dan mungkin — mungkin mereka sendiri tidak sepenuhnya saling percaya."</p>`;
    }
...
'ch4_trap_execution': {
  text: (s) => {
    ...
    text += `<p class="sound">— Tapi kau tidak sendirian. —</p>`;
    ...
    text += `<p><span class="speaker juno">Juno</span> "Gotcha," bisikmu.</p>`;
...
'ch4_force_reveal': {
  text: `<p><span class="speaker juno">Juno</span> "SEMUA ORANG KE HALL! SEKARANG!"</p>
<p class="sound">— Suaramu menggelegar di koridor. Satu per satu, mereka datang. Yang tersisa. —</p>
<p><span class="speaker juno">Juno</span> Kau berdiri di tengah lingkaran kursi — di mana semua ini dimulai — dan menatap setiap wajah.</p>
<p><span class="speaker juno">Juno</span> "Cukup. Tidak ada lagi permainan. Tidak ada lagi simpul." Kau mengangkat bukti-bukti yang kau kumpulkan. "Aku tahu siapa Sang Penenun. Aku tahu siapa operatornya. Dan aku punya bukti."</p>
<p><span class="speaker juno">Juno</span> "Jadi sekarang, Sang Penenun — siapapun kau — kau punya dua pilihan. Tunjukkan dirimu. Atau biarkan aku membuka semuanya — dan kau kehilangan kendali atas narasinya."</p>
<p class="sound">— Hening. Napas tertahan. —</p>
<p class="sound">— Detik-detik paling panjang dalam hidupmu. —</p>`,
  shake: true,
  choices: [
    {
...
'ch4_dual_mission': {
  text: (s) => {
    let text = `<p><span class="speaker juno">Juno</span> Kau membagi tugas: cari kunci DAN kumpulkan bukti terhadap Sang Penenun secara paralel.</p>`;
    text += `<p><span class="speaker sera">Sera</span> Tim A (kau, Sera) — investigasi identitas Penenun melalui bukti-bukti yang sudah dikumpulkan.</p>`;
    text += `<p><span class="speaker juno">Juno</span> Tim B (Juno + yang lain) — cari kunci master atau jalan keluar alternatif.</p>`;
    text += `<p class="sound">— Tiga puluh menit koordinasi. Lalu kalian berpencar. —</p>`;

    text += `<p>Kau dan <span class="speaker sera">Sera</span> duduk di perpustakaan, menyusun timeline:</p>`;
    text += `<p class="sound">— • 50 tahun lalu — mansion dibangun, artefak ditemukan, siklus simpul dimulai —</p>`;
    text += `<p class="sound">— • 7 tahun terakhir — acara tahunan dengan pola konsisten —</p>`;
    text += `<p><span class="speaker niko">Niko</span> • 1 tahun lalu — Hendarto Wardhana (kakek Niko) meninggal</p>`;
    text += `<p><span class="speaker vira">Vira</span> • 6 bulan lalu — Vira diundang dan "selamat"</p>`;
    text += `<p><span class="speaker niko">Niko</span> • 3 bulan lalu — Niko merenovasi mansion, menyewa "PT Benang Digital"</p>`;
    text += `<p><span class="speaker niko">Niko</span> • Malam ini — siklus ke-8, tapi seseorang MEMBAJAK acara Niko</p>`;

    text += `<p><span class="speaker niko">Niko</span> Sera: "Pola-nya berubah tahun ini. Sebelumnya, Hendarto yang menjalankan — di bawah kendali Penenun. Tapi Hendarto mati. Dan Niko tidak mau melanjutkan — dia ingin mengungkap."</p>`;
    text += `<p><span class="speaker niko">Niko</span> "Jadi Sang Penenun merekrut operator baru — Lana dan Dimas — dan membajak acara Niko."</p>`;
    text += `<p><span class="speaker sera">Sera</span> "Tapi siapa Sang Penenunnya?" . "Kalau dia mem-blackmail Hendarto selama 48 tahun, dia harus berusia minimal 70an. Dan satu-satunya orang di mansion ini yang cocok dengan profil itu..."</p>`;
    text += `<p class="sound">— Kalian menatap satu sama lain. —</p>`;
    text += `<p><span class="speaker sera">Sera</span> "Tidak ada," . "Kecuali Penenun bukan satu orang — tapi posisi yang diwariskan."</p>`;
...
'ch4_defend': {
  text: `<p><span class="speaker sera">Sera</span> "Kita tidak berlari dan kita tidak berburu. Kita bertahan. Dua jam sampai fajar — kita bisa."</p>
<p><span class="speaker sera">Sera</span> Kau memimpin barricade hall utama. Kursi ditumpuk di pintu. Jendela dikunci tambahan dengan furniture. Satu pintu masuk dijaga bergantian.</p>
<p><span class="speaker sera">Sera</span> Tapi defense punya kelemahan: kau tidak bisa mengontrol apa yang terjadi di LUAR barricade.</p>
<p><span class="speaker sera">Sera</span> Speaker: <em>"Menarik. Tikus yang membangun sarang di dalam perangkap. Tapi apakah semua tikus di dalam sarang itu... tikus?"</em></p>
<p><span class="speaker sera">Sera</span> Pertanyaan yang menghantam. Di dalam barricade kalian — apakah ada pembunuh yang terkurung bersama?</p>`,
  shake: true,
  choices: [
    {
...
'ch4_interrogation': {
  text: (s) => {
    let text = `<p><span class="speaker sera">Sera</span> Interogasi di bawah cahaya darurat. Wajah-wajah yang kelelahan, ketakutan, dan — di beberapa — bersalah.</p>`;
    ...
    text += `<p><span class="speaker vira">Vira</span> Waktu terus berjalan. Interogasi tidak menghasilkan konfesi — hanya lebih banyak kecurigaan.</p>`;
    text += `<p class="sound">— Dan kemudian — lampu mati lagi. Kegelapan selama sepuluh detik. —</p>`;
    text += `<p class="sound">— Lampu menyala. —</p>`;
    text += `<p><span class="speaker vira">Vira</span> Semua masih ada. Tapi di lantai, di tengah barricade, ada sesuatu yang tadi tidak ada: boneka kayu. Boneka yang menyerupaimu. Dengan pita merah di lehernya.</p>`;
    text += `<p class="sound">— Sang Penenun masuk ke dalam barricade — dan keluar — tanpa terdeteksi. —</p>`;
...
'ch4_systematic_search': {
  text: `<p><span class="speaker vira">Vira</span> Pencarian sistematis — kamar per kamar, lorong per lorong. Tapi mansion ini luas dan waktu menipis.</p>
...
<p class="sound">— Judul: "Simpul Terakhir". —</p>
<p><span class="speaker lana">Lana</span> Kau membaca halaman pertama: <em>"Sepuluh orang diundang ke sebuah mansion terpencil. Sebelum fajar, tiga di antaranya akan mati. Yang tersisa akan bertanya-tanya apakah mereka selamat karena keberuntungan — atau karena memang direncanakan."</em></p>
<p><span class="speaker lana">Lana</span> Ini bukan novel. Ini SKENARIO. Blueprint malam ini. Dan Lana menulisnya SEBELUM datang ke mansion.</p>
<p><span class="speaker lana">Lana</span> Kau membalik ke halaman terakhir. Ending yang direncanakan: <em>"Protagonis selamat. Membawa cerita ke dunia luar. Tapi tidak ada yang percaya. Trauma menjadi material podcast baru — dan siklus dimulai lagi saat listener berikutnya menerima undangan."</em></p>`,
  shake: true,
  choices: [
    {
...
'ch4_return_evidence': {
  text: (s) => {
    let text = `<p class="sound">— Kau kembali ke kelompok dengan bukti baru. Wajah-wajah tegang mengarah padamu. —</p>`;
    text += `<p><span class="speaker lana">Lana</span> "Aku tahu siapa operatornya," katamu. "Dan aku punya bukti."</p>`;

    if (s.flags.hasManuscript) {
      text += `<p><span class="speaker lana">Lana</span> Kau menunjukkan manuscript Lana. "Novel ini bukan fiksi — ini blueprint malam ini. Lana menulis SKENARIO, bukan novel."</p>`;
    }
    if (s.flags.hasWalkieRecording) {
      text += `<p><span class="speaker lana">Lana</span> Kau memainkan rekaman walkie-talkie. Suara-suara yang teridentifikasi memenuhi ruangan.</p>`;
    }

    text += `<p><span class="speaker lana">Lana</span> Reaksi bervariasi — tapi satu hal jelas: kebenaran sudah tidak bisa ditutup lagi.</p>`;
...
'ch4_ambush': {
  text: `<p class="sound">— Kau keluar dari hidden room — dan langsung menghadapi masalah. —</p>
<p class="sound">— Sosok gelap. Di koridor. Menunggu. —</p>
<p><span class="speaker lana">Lana</span> Bukan salah satu tamu — atau setidaknya, tidak terlihat seperti tamu. Berpakaian gelap, wajah tertutup balaclava.</p>
<p class="sound">— Dia bergerak cepat. Kau bergerak lebih cepat. —</p>
<p class="sound">— Tapi koridor sempit. Tidak ada tempat lari kecuali maju atau mundur. —</p>
...
<p><span class="speaker juno">Juno</span> Pergulatan singkat. Brutal. Juno memukul. Sosok itu memukul balik — lebih keras, lebih terlatih. Juno terjengkang ke dinding.</p>
<p><span class="speaker juno">Juno</span> Kau mengambil vas terdekat dan menghantamkannya ke kepala sosok itu. Vas pecah. Sosok itu terhuyung — lalu berlari, menghilang ke koridor gelap.</p>
<p><span class="speaker juno">Juno</span> Juno bangun, memegangi tulang rusuknya. "Orang itu bukan amatir. Terlatih. Militer atau sejenisnya."</p>`,
  shake: true,
  choices: [
    {
...
'ending_coward_escape': {
  ...
  rating: 'D',
  endingText: (s) => {
    return `<p class="sound">— Kau berlari ke malam. Sendirian. Meninggalkan semua orang di dalam. —</p>
<p><span class="speaker juno">Juno</span> Kau menemukan jalan. Kau menghubungi polisi. Mereka datang — tapi sudah terlambat. Dua orang lagi meninggal sebelum fajar.</p>
<p><span class="speaker juno">Juno</span> Polisi tidak menemukan bukti kejahatan — mansion sudah dibersihkan. Kasus ditutup sebagai "kecelakaan selama acara privat".</p>
<p><span class="speaker juno">Juno</span> Kau hidup. Tapi podcast-mu mati. Tidak ada yang mau mendengarkan cerita dari orang yang meninggalkan teman-temannya untuk mati.</p>
<p><span class="speaker juno">Juno</span> Setiap malam, kau mendengar suara-suara mereka. Meminta tolong. Bertanya kenapa kau lari.</p>
<p class="sound">— Kau hidup — tapi itu bukan sama dengan selamat. —</p>`;
  },
  fates: {
